...
void experiment(InitFun init_f, RangeFun range_f, SizeFun size_f, const double param, InputKeys<key_type> &keys, Workload<key_type> &queries, Args... args)
{
    ...
    // for testing false negative
    // for (auto q : keys)
    // {
    //     bool query_result = range_f(f, q, q + 1000);
    //     if (query_result)
    //         fp++;
    // }
...
void init_parser(argparse::ArgumentParser &parser)
{
    ...
    parser.add_argument("--range-size")
            .help("the max range size in queries")
            .nargs(1)
            .scan<'i', int>();

    parser.add_argument("--corr-degree")
            .help("the correlated degree in keys and queries")
            .nargs(1)
            .scan<'i', int>();
}

std::tuple<InputKeys<uint64_t>, Workload<uint64_t>, double, int, int> read_parser_arguments(argparse::ArgumentParser &parser)
{
    auto range_size = parser.get<int>("range-size");

    auto corr_degree = parser.get<int>("corr-degree");
    ...
    return std::make_tuple(keys, queries, arg, range_size, corr_degree);
}
