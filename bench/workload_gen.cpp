...
std::vector<double> calculateZipfianProbabilities(const size_t n_unique, double alpha) {
    std::vector<double> probabilities(n_unique, 0.0);
    double sum = 0.0;
...
std::vector<double> calculatePowerLawProbabilities(const size_t n_unique, double alpha) {
    std::vector<double> probabilities(n_unique, 0.0);
    double sum = 0.0;
...
Workload<uint64_t> generate_synth_queries(const std::string& qdist, InputKeys<uint64_t> &keys,
    ...
    else if (qdist == "quniform" || qdist == "qadapt-uniform")
        middle_points = generate_keys_uniform(3 * n_queries);
    else // qdist == "qcorrelated" or "qadapt-corr"
    {
        std::mt19937 g(seed);
        auto t = keys;
    ...
    while (q.size() < n_queries) {
        ...
        if (qdist == "quniform" || qdist == "qadapt-uniform")
        ...
        else // qdist == qcorrelated or qadapt-corr(as = 0)
        {
    ...
    if ((qdist == "qadapt-uniform" || qdist == "qadapt-corr") && as_ratio != 0) {
        ...
        std::vector<std::tuple<uint64_t, uint64_t, bool>> repeated_q;
        auto query_pool = std::vector<std::tuple<uint64_t, uint64_t, bool>>(q.begin(), q.end());
        repeated_q.reserve(n_q);

        for (int i = 0; i < n_q; ++i) {
            auto query = query_pool[zipf_distribution(zipf_generator)];
            repeated_q.push_back(query);
            printProgress(((double) repeated_q.size()) / n_q);
        }
        return repeated_q;
    }
...
generate_real_queries(std::vector<uint64_t> &data, uint64_t n_keys, uint64_t n_queries, std::vector<uint64_t> &range_list, bool remove_duplicates = true) {
    ...
    if (data.size() < n_queries)
        throw std::runtime_error(
                "error, can build at most " + std::to_string(data.size()) + " over " + std::to_string(n_queries) +
                " queries");
    ...
    std::cout << "[+] found " << candidates.size() << " query candidates." << std::endl;
    ...
    // select 16M uniform from real keys
    std::mt19937 gen(seed);
    std::uniform_int_distribution<uint64_t> distr_value(0, keys.size() - 1);

    std::set<uint64_t> sample_keys;
    auto n_iterations = 0;
    while (sample_keys.size() < n_keys) {
        if (++n_iterations >= 10 * n_keys)
            throw std::runtime_error("error: timeout for the input keys generation");

        auto key = keys[distr_value(gen)];
        sample_keys.insert(key);
        printProgress(((double) sample_keys.size()) / n_keys);
    }
    ...
    return std::make_pair(std::vector<uint64_t>(sample_keys.begin(), sample_keys.end()), queries_list);
}

template <typename value_type = uint64_t>
void generate_real_dataset(const std::string& file, uint64_t n_keys, uint64_t n_queries, std::vector<int> range_size_list) {
    std::vector<uint64_t> ranges(range_size_list.size());
    std::transform(range_size_list.begin(), range_size_list.end(), ranges.begin(), [](auto v) {
        return (1ULL << v);
    });
    ...
    auto [keys, queries_list] = generate_real_queries(all_data, n_keys, n_queries, ranges);
    std::cout << std::endl << "[+] full dataset generated" << std::endl;
    std::cout << "[+] nkeys=" << n_keys << ", nqueries=" << queries_list[0].size() << std::endl;
    ...
    for (auto i = 0; i < ranges.size(); i++) {
        ...
        for (auto tuple : queries_list[i]) {
            auto [l, r, c] = tuple;
            auto bound = std::lower_bound(keys.begin(), keys.end(), l);
            assert(bound == keys.end() || *bound > r);
        }
    }
