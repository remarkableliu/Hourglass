...
generate_adapt_test() {
  ...
  while [ $i -le 4 ]
  do
    $WORKLOAD_GEN_PATH --kdist kuniform --qdist qadapt-uniform --as-ratio ${x}
    mv kuniform/ kuniform_${x}/
    # $WORKLOAD_GEN_PATH --kdist kuniform --qdist qadapt-corr --as-ratio ${x}
...
# if ! $WORKLOAD_GEN_PATH --mixed --kdist knormal; then
#   echo "[!!] fpr_test generation failed"
#   exit 1
# fi
# if ! $WORKLOAD_GEN_PATH --mixed --kdist kuniform; then
#   echo "[!!] fpr_test generation failed"
#   exit 1
# fi
...
# if ! $WORKLOAD_GEN_PATH --binary-keys $REAL_DATASETS_PATH/fb_200M_uint64 ; then
...
# mkdir -p ../fpr_real_test && cd ../fpr_real_test || exit 1
# if ! $WORKLOAD_GEN_PATH --binary-keys $REAL_DATASETS_PATH/books_200M_uint64 $REAL_DATASETS_PATH/fb_200M_uint64 $REAL_DATASETS_PATH/osm_cellids_200M_uint64 || \
#   ! $WORKLOAD_GEN_PATH --range-size 0 --binary-keys $REAL_DATASETS_PATH/wiki_ts_200M_uint64 || \
#   ! $WORKLOAD_GEN_PATH -q 600000 --range-size 5 --binary-keys $REAL_DATASETS_PATH/wiki_ts_200M_uint64 || \
#   ! $WORKLOAD_GEN_PATH -q 6000 --range-size 10 --binary-keys $REAL_DATASETS_PATH/wiki_ts_200M_uint64; then
#   echo "[!!] fpr_real_test generation failed"
#   exit 1
# fi
# echo "[!!] fpr_real_test (figure 4) dataset generated"
