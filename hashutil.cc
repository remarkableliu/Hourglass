...
__uint128_t HashUtil::MurmurHash3 ( const void * buf, size_t length,
                           const uint32_t seed)
{
  ...
  return h1 + (__uint128_t(h2) << 64);
}

__uint128_t HashUtil::MurmurHash3(const std::string &s, uint32_t seed) {
  return MurmurHash3(s.data(), s.length(), seed);
}
