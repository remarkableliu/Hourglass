...
template <util::AllocType AT = util::AllocType::MALLOC> class Rank9 : public Rank {
  ...
  public:
	...
	/** Returns the bit in position k of this structure. */
	size_t getBit(const size_t pos) { return bits[pos / 64] >> pos % 64 & 0x1; }

	/** Returns the number of one bits in bit vector. */
	uint64_t getNumOnes() { return num_ones; }

	/** Returns an estimate of the size in bits of this structure. */
	size_t bitCount() const { return counts.bitCount() - sizeof(counts) * 8 + sizeof(*this) * 8 + num_bits; }
