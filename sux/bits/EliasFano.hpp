...
template <util::AllocType AT = util::AllocType::MALLOC> class EliasFano : public Rank {
  private:
  ...
  public:
	...
	/** Returns the bit in position k of the underlying bit vector. */
	size_t getBit(const size_t pos) { return rank(pos + 1) - rank(pos); }

	/** Returns the number of one bits in bit vector. */
	uint64_t getNumOnes() { return num_ones; }
	...
	size_t bitCount() const {
		return upper_bits.bitCount() - sizeof(upper_bits) * 8 + lower_bits.bitCount() - sizeof(lower_bits) * 8 + selectz_upper.bitCount() -
